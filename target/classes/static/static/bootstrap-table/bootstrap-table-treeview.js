...
(function ($) {
    ...
    BootstrapTable.prototype.initData = function () {
        ...
        if (that.options.treeView && this.data.length > 0) {
            ...
            var g = function(child, level){
                var childLevel = that.options.treeRootLevel + level;
                $.each(child, function (i, n) {
                    n.level = childLevel;
            ...
            if (column.checkbox || column.radio) {
                ...
                value = typeof value === 'undefined' || value === null ?
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        indent = item.level == that.options.treeRootLevel ? '' : sprintf('<span style="margin-left: %spx;"></span>', (item.level - that.options.treeRootLevel) * 30);
