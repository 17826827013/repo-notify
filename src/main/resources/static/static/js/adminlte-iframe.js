// baseurl
var baseurl = document.getElementById("content_baseurl").value;
/**
...
var addTabs = function (options) {
    ...
    if (findTabPanel(pageId) == null) {
        ...
        if (options.icon) {
            $("<span class='" + options.icon + "' style='padding-right: 3px;'></span>").appendTo($title);
        }
        ...
        //是否允许关闭
        if (options.close) {
            var $i = $("<i class='fa fa-remove page_tab_close' style='cursor: pointer;padding-left: 3px;' onclick='closeTab(this);'></i>").attr(pageIdField, pageId).appendTo($title);
...
function closeTabByPageId(pageId) {
    ...
    if ($title.hasClass("active")) {
        ...
        if ($nextTitle.length > 0) {
            ...
            var $prevTitle = $title.prev();
            if ($prevTitle.length > 0) {
                activePageId = getPageId($prevTitle);
            } else {
                return;
            }
        }
...
var refreshTab = function () {
    refreshTabById(getActivePageId());
};
...
$(function () {
    $(".menuTabs").on("click", ".menu_tab", function () {
        activeTabByPageId(getPageId(this));
    });
});
(function ($) {
    $.fn.sidebarMenu = function (options) {
        ...
        function init($menu_ul, data, level) {
            $.each(data, function (i, item) {
                ...
                if (item.icon) {
                    var $icon = $('<i></i>');
                    if (item.icon.length > 1 && item.icon.substr(0,2) == 'fa') {
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    if (item.targetType != null) {
                        ...
                        else if (item.targetType === "iframe") { //代表单iframe页面
                            $a.attr("href", item.url);
    ...
    $.fn.sidebarMenu.defaults= {url: null,param: null,data: null,type: null};
})(jQuery);